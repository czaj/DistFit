...
% unbounded 
    ...
    case 3 % Generalized Extreme Value % k, sigma, mu 
        ...
        p = pnormcdf(b0(4))*(p0(:,2) - p0(:,1))+(1-normcdf(b0(4)));
    ...
    case 4 % tLocationScale % mu, sigma, nu 
        ...
        p = normcdf(b0(4))*(p0(:,2) - p0(:,1))+(1-normcdf(b0(4)));
...
% bounded (0,Inf)
    case 10 % Exponential % mu
        ...
        p = normcdf(b0(2))*(p0(:,2) - p0(:,1))+(1-normcdf(b0(2)));
        p(p==0) = pdf('Exponential',bounds(p==0,1),b0(1));
        f = log(p);
    ...
    case 14 % Rayleigh % b0
        ...
        p = normcdf(b0(2))*(p0(:,2) - p0(:,1))+(1-normcdf(b0(2)));
        p(p==0) = pdf('Rayleigh',bounds(p==0,1),b0(1));
        f = log(p);
    case 15 % Gamma % a, b
...
%         f = log(p);
 ...
 % discrete
    case 31 % Poisson % lambda
        ...
        p = normcdf(b0(2))*(p0(:,2) - p0(:,1))+(1-normcdf(b0(2)));
        p(p==0) = pdf('Poisson',bounds(p==0,1),b0(1));
        f = log(p);
