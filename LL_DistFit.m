...
switch dist
    ...
    case 3 % Generalized Extreme Value % k, sigma>0, mu
        ...
        dp(dp==0) = pdf('Generalized Extreme Value',bounds_min(dp==0,1),bDist(1),bDist(2),bDist(3));
    case 4 % tLocationScale % mu, sigma>0, nu>0
        ...
        dp(dp==0) = pdf('tLocationScale',bounds_min(dp==0,1),bDist(1),bDist(2),bDist(3));
    case 5 % Uniform % min, max
        %         save tmp2
        %         return
        dp = cdf('Uniform',bounds(:,2),bDist(1),bDist(2)) - ...
            cdf('Uniform',bounds(:,1),bDist(1),bDist(2));
        dp(dp==0) = pdf('Uniform',bounds_min(dp==0,1),bDist(1),bDist(2));
    ...
    case 10 % Exponential % mu>0
        ...
        dp(dp==0) = pdf('Exponential',bounds_min(dp==0,1),bDist(1));
    ...
    case 16 % BirnbaumSaunders % beta>0, gamma>0
        ...
        dp(dp==0) = pdf('BirnbaumSaunders',bounds_min(dp==0,1),bDist(1),bDist(2));
    case 17 % Generalized Pareto % k, sigma>0, theta
        ...
        dp(dp==0) = pdf('Generalized Pareto',bounds_min(dp==0,1),bDist(1),bDist(2),bDist(3));
    case 18 % InverseGaussian % k>0, sigma>0
        ...
        dp(dp==0) = pdf('InverseGaussian',bounds_min(dp==0,1),bDist(1),bDist(2));
...
%         f = log(p).*weights;
    case 20 % Rician % s>=0, sigma>0
        ...
        dp(dp==0) = pdf('Rician',bounds_min(dp==0,1),bDist(1),bDist(2));
    ...
    case 31 % Poisson % lambda>0
        ...
        dp(dp==0) = pdf('Poisson',bounds_min(dp==0,1),bDist(1));
    case 32 % Negative Binomial % R>0 and R is an integer, 0<P<1
        ...
        dp(dp==0) = pdf('Negative Binomial',bounds_min(dp==0,1),bDist(1),bDist(2));
